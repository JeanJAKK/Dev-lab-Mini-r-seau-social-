...
export default function PostDetail() {
  ...
  const getAvatarUrl = (profile) => {
    if (!profile)
      return `https://ui-avatars.com/api/?name=U&background=7c3aed&color=fff`;
  ...
  const parseComment = (content) => {
    ...
    if (tabIdx > 0 && content.startsWith("@")) {
      return {
        mention: content.slice(1, tabIdx),
        text: content.slice(tabIdx + 1),
      };
    }
    ...
    const init = async () => {
      ...
      if (!error && postData) {
        const { data: likesData } = await supabase
          ...
          .select("*")
          .eq("post_id", postId);
        const liked = likesData?.some((l) => l.user_id === currentUserId) || false;
  ...
  const handleReply = (comment) => {
    setReplyingTo({
      id: comment.id,
      name: comment.profiles?.name || "Utilisateur",
    });
    setCommentText("");
  ...
  const handleSubmit = async () => {
    ...
    await sendComment(currentUserId, postId, content);
  ...
  const handleDownloadImage = () => {
    if (!post?.image_url) {
      setShareNotice("Aucune image à télécharger.");
      setTimeout(() => setShareNotice(""), 2200);
      return;
    }
    ...
    try {
      ...
      link.click();
      link.remove();
      setShareNotice("Téléchargement lancé.");
      setTimeout(() => setShareNotice(""), 2200);
    } catch {
      setShareNotice("Impossible de télécharger cette image.");
      setTimeout(() => setShareNotice(""), 2200);
    }
  };
    ...
    <div className={`fixed inset-0 z-50 flex flex-col overflow-hidden ${isDark ? "bg-gray-900" : "bg-white"}`}>
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        ...
        <div className={`flex flex-col flex-1 md:flex-none overflow-hidden ${post.image_url ? "md:w-[380px]" : ""} ${isDark ? "bg-gray-900 border-l border-gray-700" : "bg-white border-l border-gray-200"}`}>
          ...
          {/* Bouton retour desktop (sans image) */}
          {!post.image_url && (
          ...
          {/* Auteur */}
          <div className={`flex items-center justify-between gap-3 px-4 py-3 border-b shrink-0 ${isDark ? "border-gray-700" : "border-gray-100"}`}>
            <Link to={`/home/profile/${post.user_id}`} className="flex items-center gap-3 flex-1">
              <img
            ...
            <div className="relative" ref={menuRef}>
              ...
              {isMenuOpen && (
                <div className={`absolute right-0 top-10 z-20 min-w-[190px] rounded-xl border shadow-lg py-1 ${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}>
                  {post.image_url && (
                    <button
                      onClick={() => {
                        handleDownloadImage();
